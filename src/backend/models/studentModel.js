...
static async getStudentsWithExpediente(grado, grupo) {
    try {
      const [results] = await db.query(
        `
        SELECT e.curp, e.nombres, e.apellido_paterno, e.apellido_materno,
               COUNT(ex.id_expediente) AS expediente_count
        FROM estudiante e
        LEFT JOIN expediente ex ON e.curp = ex.id_estudiante
        WHERE e.grado = ? AND e.grupo = ?
        GROUP BY e.curp
        HAVING expediente_count > 0
        `,
        [grado, grupo]
      );
      console.log('Students with expediente:', results);
      return results;
    } catch (error) {
