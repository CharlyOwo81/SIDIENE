  // src/pages/ViewRecord.tsx
  import { useState, useEffect } from 'react';
  import { motion } from 'framer-motion';
  import { ExpedienteService, Expediente, Incidencia, Acuerdo } from '../../../backend/services/expedienteService';
  ...
  const ViewRecord = () => {
    ...
    // Obtener estudiantes con expedientes
    useEffect(() => {
    ...
    // Obtener expediente al seleccionar estudiante
// src/pages/ViewRecord.tsx
const handleSelectStudent = async (student: {
  ...
  try {
    ...
    console.log('ExpedienteService response:', response);
    if (response.success) {
      setSelectedExpediente(response.data?.[0] || null);
    } else {
      setAlert({ message: response.message || 'Expediente no encontrado', type: 'warning' });
    }
  } catch (error) {
    ...
    return (
      ...
      >
        ...
        >
          <h1 className={styles.formTitle}>Consultar Expediente Único de Incidencias</h1>
          ...
          >
            {!loading && filters.grade && filters.group && students.length === 0 && (
              <div className={styles.emptyState}>No hay estudiantes con expediente en este grado y grupo.</div>
            )}
            ...
            {students.map((student) => (
              ...
              >
                ...
                <div className={styles.badge}>
                  {student.expediente_count} incidencia{student.expediente_count !== 1 ? 's' : ''}
                </div>
              </motion.div>
            ))}
          </motion.div>
          ...
          {selectedStudent && !loading && (
            ...
            >
              <h2 className={styles.expedienteTitle}>
                Expediente de{' '}
                {`${selectedStudent.nombres} ${selectedStudent.apellido_paterno} ${
                  selectedStudent.apellido_materno || ''
                }`.trim()}
              </h2>
              ...
              {selectedExpediente ? (
                  ...
                  {selectedExpediente.incidencias.map((incidencia: Incidencia) => (
                    ...
                    >
                      <div className={styles.incidentHeader}>
                        <div className={styles.severityBadge} data-severity={incidencia.nivel_severidad}>
                          {incidencia.nivel_severidad}
                        </div>
                        <span>{new Date(incidencia.fecha).toLocaleDateString()}</span>
                      </div>
                        ...
                        {incidencia.acuerdos.length > 0 && (
                          <div className={styles.acuerdoSection}>
                            ...
                            {incidencia.acuerdos.map((acuerdo: Acuerdo) => (
                              <div key={acuerdo.id_acuerdo} className={styles.acuerdoItem}>
                                <div className={styles.acuerdoStatus} data-status={acuerdo.estatus}>
                                  {acuerdo.estatus}
                                </div>
                                <p>{acuerdo.descripcion}</p>
                                <small>Creado el: {new Date(acuerdo.fecha_creacion).toLocaleDateString()}</small>
                              </div>
                            ))}
                          </div>
                        )}
                ...
                <div className={styles.emptyState}>No se encontró expediente para este estudiante.</div>
              )}
            </motion.div>
          )}
        </motion.div>
